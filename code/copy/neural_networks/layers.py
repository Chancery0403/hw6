...
class FullyConnected(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int, int]) -> None:
        ...
        #copied code from init_parameters in Conv2D class
...
class BatchNorm1D(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int, int]) -> None:
        ...
        self.cache = OrderedDict({"X": [], "X_hat": [], 
                                  "mu": np.zeros((1, self.n_in)), "var": np.zeros((1, self.n_in)), 
                                  "running_mu": np.zeros((1, self.n_in)), "running_var": np.zeros((1, self.n_in))})  
        # cache for backprop
        self.gradients: OrderedDict = OrderedDict({"gamma": np.zeros_like(gamma), "beta": np.zeros_like(beta)})  # parameter gradients initialized to zero
                                           # MUST HAVE THE SAME KEYS AS `self.parameters`
    ...
    def forward(self, X: np.ndarray, mode: str = "train") -> np.ndarray:
        ...
        if mode == "train":
            ...
            mu_tilde = np.mean(h, axis=0)
            sigma_tilde = np.mean(np.square(h - mu_tilde), axis=0)
            #running mean updated
            a = ((1-self.momentum) * mu_tilde) + ((self.momentum) * self.cache["running_mu"])
            self.cache["running_mu"] = a
            #running variance updated
            b = ((1-self.momentum) * sigma_tilde) + ((self.momentum) * self.cache["running_var"])
            self.cache["running_var"] = b
            h_tilde = (h - mu_tilde) / np.sqrt(sigma_tilde + 0.00000001)

            self.cache["X_hat"] = h_tilde
            self.cache["mu"] = mu_tilde
            self.cache["var"] = sigma_tilde
        else:
            h_tilde = (h - self.cache["running_mu"]) / np.sqrt(self.cache["running_var"]+ 0.00000001)
        #out
        out = self.parameters["gamma"] * h_tilde + self.parameters["beta"]
